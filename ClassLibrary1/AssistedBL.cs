    ...
    public class AssistedBL
    {
        ...
        public static bool SendMessagesToAssisteds(List<EmbeddindAssisted> embeddindAssisteds)
        {
            try
            {
                ...
                db.SaveChanges();
                string body = "השיבוץ בוצע בהצלחה להלן פרטי התקשרות עם הנעזר";
                foreach (var item in embeddindAssisteds)
                {
                    body = body + item.full_name + "פלאפון:" + item.phone;
                }
                Mail.SendMail(vEmail, "אשרייך!", body);
                volunteer v1 = db.volunteer.FirstOrDefault(x => x.id_volunteer == embeddindAssisteds[0].id_volunteer);
                string body1 = "מצאנו לך את המתנדב המתאים להלן פרטי התקשרות עם המתנדב" + "שם:" + v1.firstName_volunteer + "" + v1.lastName_volunteer + "פלאפון:" + v1.phone;
